...
CAM_NAMES = ["cam0%d" % cam_num for cam_num in (1, 3, )]
SPLITS = {
...
class OnceImageLidarDataset(Dataset):
    ...
    def _setup(self, split: str) -> torch.Tensor:
        ...
        for attr in SPLITS[split]:
            seq_list_path = os.path.join(os.path.dirname(self._data_root), "ImageSets", f"{attr}.txt")
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Failed to load point cloud {sequence_id}/{frame_id}/{cam_name}"
            )
