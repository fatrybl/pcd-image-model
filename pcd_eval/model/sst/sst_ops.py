...
from torchex import ingroup_inds

import numpy as np

get_inner_win_inds = ingroup_inds
...
def flat2window(feat, voxel_drop_lvl, flat2win_inds_dict, drop_info, padding=0):
    ...
    for dl in drop_info:
        ...
        padding = torch.as_tensor(padding, dtype=dtype, device=device)
...
def make_continuous_inds(inds):
    ...
    # canvas = -torch.ones(max_origin_inds+1)
